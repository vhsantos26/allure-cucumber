...
module AllureCucumber
  ...
  class Formatter
    ...
    def before_scenario(scenario); end
    ...
    def before_test_step(test_step)
      unless TEST_HOOK_NAMES_TO_IGNORE.include?(test_step.text) 
    ...
    def after_test_step(test_step, result)
      ...
      unless TEST_HOOK_NAMES_TO_IGNORE.include?(test_step.text)
        if @tracker.scenario_name
          ...
          @deferred_after_test_steps << {
            step: test_step, result: result, timestamp: Time.now
          }
        end
      end
    end
    ...
    def step_status(result)
      POSSIBLE_STATUSES.each do |status|
        cucumber_status_to_allure_status(status) if result.send("#{status}?")
      end
    end
    ...
    def test_result(result)
      ...
      exception ? { status: status, exception: exception } : { status: status }
    end
    ...
    def attach_multiline_arg_to_file(multiline_arg)
      ...
      File.open(out_file, 'w+') do |file|
        file.write(multiline_arg.to_s.gsub(/\e\[(\d+)(;\d+)*m/, ''))
      end
